//#region node_modules/.nitro/vite/services/ssr/assets/ai-gateway.server-BXTUySJa.js
var GATEWAY_URL = "https://ai.gateway.lovable.dev/v1/chat/completions";
var SYSTEM_PROMPT = `You are a precise invoice data extractor. Given an invoice image or PDF, extract these fields and return ONLY a JSON object — no prose, no markdown fences.
...
async function extractInvoiceFromFile(args) {
	const apiKey = process.env.LOVABLE_API_KEY;
	if (!apiKey) throw new Error("LOVABLE_API_KEY is not configured");
	const dataUrl = `data:${args.mimeType};base64,${args.base64}`;
	const userContent = [{
		type: "text",
		text: "Extract the invoice fields. Return only the JSON object."
	}];
	if (args.mimeType.startsWith("image/")) userContent.push({
		type: "image_url",
		image_url: { url: dataUrl }
	});
	else if (args.mimeType === "application/pdf") userContent.push({
		type: "image_url",
		image_url: { url: dataUrl }
	});
	else throw new Error(`Unsupported MIME type for extraction: ${args.mimeType}`);
